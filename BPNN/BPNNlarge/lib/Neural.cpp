...
//	printf("wgtnum=%d\n", wgtnum);
	for(int wpin=0; wpin<wgtnum; wpin++){
		...
		wgt[wpin] += 0.0001 * ratio * attrset[wpin] * err * prt
			* abs(attrset[wpin] * err * prt);// / 10000;
	}
	/*printf("error=%lf\n", geterr());
	printf("prt=%lf\n", diff(foretell));
	printf("\delta wgt=%lf\n", geterr()*diff(foretell));*/
	//bias += geterr()*diff(foretell);
	bias = 0;
//	printf("bias=%lf\n", bias);
...
//	nxtlayer[0]->dissec();
	for(int xpin=0; xpin<nxtlayer.size(); xpin++){
		...
		nxtlayer[xpin]->Learn();
//		cout << nxtlayer[xpin]->wgtnum << endl;
	}
	...
	for(int npin=0; npin<neulayer.size(); npin++){
		...
		neulayer[npin].Learn(0.1*(npin+1));
	}
